...
(() => {
  ...
  catch { store.notice('לא ניתן לקרוא את הגדרות הסנכרון. ניתן להגדיר אותן מחדש.'); }
  ...
  async function api(path, method = 'GET', body) {
    ...
    if (!response.ok) throw Error(response.status === 401 ? 'אסימון הגישה לא תקין או שפג תוקפו' : response.status === 404 ? 'ה־Gist לא נמצא או שאין אליו גישה' : `GitHub החזיר שגיאה ${response.status}`);
    return response.json();
  }
  function parseGist(gist) {
    if (gist.public !== false) throw Error('יש לבחור Gist סודי');
    if (gist.truncated) throw Error('רשימת קובצי הסנכרון גדולה מדי. יש לייצא גיבוי וליצור Gist חדש');
    let merged = {schemaVersion: 1, entries: {}};
    const files = Object.entries(gist.files || {}).filter(([name]) => /^gaming-catalog-[a-f0-9]+\.json$/.test(name));
    if (!files.length) throw Error('ה־Gist אינו מכיל גיבוי של Gaming Catalog');
    for (const [, file] of files) {
      if (file.truncated || typeof file.content !== 'string') throw Error('קובץ הסנכרון גדול מדי לקריאה. יש לייצא גיבוי וליצור Gist חדש');
  ...
  async function sync() {
    ...
    try {
      ...
      if (!store.apply(merged)) throw Error('השמירה המקומית נכשלה; ייצאו גיבוי');
      const content = JSON.stringify(store.get());
      if (content !== lastWritten) {
        if (new Blob([content]).size > 950000) throw Error('הגיבוי גדול מדי לסנכרון. ייצאו אותו לקובץ');
        await api('/' + config.id, 'PATCH', {files: {[filename]: {content}}});
        lastWritten = content;
      }
      status('☁ מסונכרן');
    } catch (error) {
      status('☁ הסנכרון נכשל'); store.notice('סנכרון: ' + error.message + '. השמירה המקומית ממשיכה לפעול.');
    } finally { busy = false; if (pending) { pending = false; schedule(); } }
  }
  function schedule() { if (!token || !config.id) return; status('☁ ממתין לסנכרון'); clearTimeout(timer); timer = setTimeout(sync, 1800); }
  function open() {
    ...
    async function connect(create) {
      if (busy) { message.textContent = 'הסנכרון הקודם עדיין מתבצע'; return; }
      const nextToken = tokenInput.value.trim(), nextId = idInput.value.trim();
      if (!nextToken) { message.textContent = 'יש להזין אסימון גישה'; return; }
      if (!create && !/^[a-f0-9]{20,64}$/i.test(nextId)) { message.textContent = 'יש להזין מזהה Gist תקין'; return; }
      if (create && nextId) { message.textContent = 'כבר הוזן מזהה. לחיבור אליו בחרו חיבור וסנכרון; ליצירה חדשה יש לפנות את השדה'; return; }
      ...
      try {
        ...
        message.textContent = 'מחובר. במכשיר אחר יש להזין את המזהה: ' + config.id;
    ...
    document.getElementById('disconnectGist').onclick = () => {
      if (busy) { message.textContent = 'יש להמתין לסיום הסנכרון'; return; }
      token = ''; config = {}; clearTimeout(timer);
      try { localStorage.removeItem(CONFIG); sessionStorage.removeItem(SESSION); } catch {}
      tokenInput.value = ''; idInput.value = ''; status('☁ סנכרון'); message.textContent = 'מנותק. השמירה המקומית וה־Gist נשארו ללא מחיקה.';
    };
  ...
  else if (config.id) status('☁ נדרש אסימון');
})();
